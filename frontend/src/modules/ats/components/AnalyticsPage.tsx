...
function SparkLine({ data, valueKey }: { data: Record<string, any>[]; valueKey: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: H }}>
      <polyline
        ...
        fill="var(--foreground)" fillOpacity={0.06} stroke="none"
      />
      <polyline
        points={polyline}
        fill="none" stroke="var(--foreground)" strokeOpacity={0.55} strokeWidth={1.5}
        strokeLinejoin="round" strokeLinecap="round"
      />
      {values.map((_, i) => {
        const [x, y] = points[i].split(',').map(Number);
        return (
          <circle key={i} cx={x} cy={y} r={3} fill="var(--foreground)" fillOpacity={0.7} />
        );
      })}
    </svg>
  );
...
export function AnalyticsPage({ candidates }: Props) {
  const total = candidates.length;
  const active = candidates.filter(c => !['Selected', 'Rejected'].includes(c.status)).length;
  const shortlisted = candidates.filter(c =>
    ['Technical Interview', 'PTC Interview', "Founder's Interview"].includes(c.status)
  ).length;
  ...
  // Pipeline stages
  const stages = ['Screening', 'Fitment Evaluation', 'Technical Interview', 'PTC Interview', "Founder's Interview", 'Selected', 'Rejected'] as const;
  ...
  // Dept distribution (top 12)
  const deptMap: Record<string, number> = {};
  candidates.forEach(c => { deptMap[c.department] = (deptMap[c.department] || 0) + 1; });
  ...
  // Avg exp by dept (top 10)
  const deptExpMap: Record<string, number[]> = {};
  candidates.forEach(c => {
    if (!deptExpMap[c.department]) deptExpMap[c.department] = [];
    deptExpMap[c.department].push(c.experience);
  });
  const deptExpData = Object.entries(deptExpMap)
    .map(([dept, exps]) => ({
      dept,
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <div className="space-y-2.5">
            {roleData.map(({ role, count }) => (
              <HBar key={role} label={role} value={count} max={maxRole} />
            ))}
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <div className="space-y-2.5">
            {deptExpData.map(({ dept, avg }) => (
              <HBar key={dept} label={dept} value={avg} max={maxAvgExp} sub="yrs avg" />
            ))}
          </div>
        </div>
      </div>
