...
export function CandidateCard({ candidate, onClick }: CandidateCardProps) {
  const [{ isDragging }, drag] = useDrag(() => ({
    type: 'CANDIDATE',
    item: { id: candidate.id, status: candidate.status, name: candidate.name },
    collect: (monitor) => ({
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-1.5">
          {isMultiRole && (
            <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold tracking-wide border border-border/60 text-muted-foreground bg-muted/20">
              MULTI-ROLE
            </span>
          )}
...
<div className="flex items-center justify-between gap-2 mb-2.5">
  ...
  {/* Source Badge */}
  <span
    className={`inline-flex items-center px-2 py-1 rounded-md text-[9px] font-semibold tracking-wide border whitespace-nowrap ${
  ...
  <div className="flex flex-wrap justify-end gap-1 max-w-[65%]">
    {candidate.skills.slice(0, 2).map((skill) => (
      <span
        key={skill}
        className="px-1.5 py-0.5 bg-muted/30 rounded text-[9px] font-medium text-muted-foreground"
      >
        {skill}
      </span>
    ))}
